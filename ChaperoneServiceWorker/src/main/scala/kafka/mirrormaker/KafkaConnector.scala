...
import kafka.chaperone.ZkOffsetStore
...
class KafkaConnector(private val consumerIdString: String,
                     ...
                     private val chaperoneZkOffsetStore: ZkOffsetStore) extends KafkaMetricsGroup {
  ...
  private def fetchOffsetFromZooKeeper(topicPartition: TopicAndPartition) = {
    ...
    val offsetString =
      if (chaperoneZkOffsetStore == null) {
        zkUtils.readDataMaybeNull(dirs.consumerOffsetDir + "/" + topicPartition.partition)._1
      } else {
        // Auditing is enabled. All offset commit is done by Chaperone
        ZkUtils.apply(chaperoneZkOffsetStore.getZkClient(), false).readDataMaybeNull(dirs.consumerOffsetDir + "/" + topicPartition.partition)._1
      }
