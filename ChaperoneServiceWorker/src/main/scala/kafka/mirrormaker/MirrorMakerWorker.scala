...
object MirrorMakerWorker extends Logging with KafkaMetricsGroup {
  ...
  // for chaperone auditing
  private var doMsgAudit = false
  private var auditorCount = 0
  private var verificationStore: VerificationStore = null
  private var auditors: Array[MessageAuditor] = null
  private var zkOffsetStore: ZkOffsetStore = null
  ...
  def main(args: Array[String]) {
    ...
    val chaperoneConfigOpt = parser.accepts("chaperone.config",
      "Embedded chaperone config.")
      .withRequiredArg()
      .describedAs("config file")
      .ofType(classOf[String])
    ...
    CommandLineUtils.checkRequiredArgs(parser, options, consumerConfigOpt, producerConfigOpt, helixConfigOpt, chaperoneConfigOpt)
    ...
    val chaperoneProps = Utils.loadProps(options.valueOf(chaperoneConfigOpt))
    initChaperone(chaperoneProps, consumerConfig)

    // TopicPartition starting offset should be obtained from chaperone zkOffsetStore
    // instead of consumerConfig.zkServers, though both can be set to be same
    connector = new KafkaConnector(consumerIdString, consumerConfig, zkOffsetStore)
  ...
  def initChaperone(chaperoneProps: Properties, consumerConfig: ConsumerConfig): Unit = {
    info("Set chaperone config=" + chaperoneProps)

    doMsgAudit = chaperoneProps.getProperty("doMsgAudit", "true").toBoolean
    val auditTopicName = chaperoneProps.getProperty("auditTopicName", "chaperone-audit")

    val auditTier = chaperoneProps.getProperty("auditTier", "staging")
    val auditDatacenter = chaperoneProps.getProperty("auditDatacenter", "staging")
    val hostMeta = new HostMetadata(auditTier, auditDatacenter)

    auditorCount = chaperoneProps.getProperty("auditorCount", "1").toInt
    val queueSizeForMessageAuditor = chaperoneProps.getProperty("queueSizeForMessageAuditor", "1024").toInt
    val initialMapSize = chaperoneProps.getProperty("initialMapSize", "2048").toInt
    val timeBucketIntervalInSec = chaperoneProps.getProperty("timeBucketIntervalInSec", "600").toLong
    val reportFreqBucketCount = chaperoneProps.getProperty("reportFreqBucketCount", "288").toInt
    val reportFreqIntervalInMs = chaperoneProps.getProperty("reportFreqIntervalInMs", "60000").toLong

    val queueSizeForVerificationStore = chaperoneProps.getProperty("queueSizeForVerificationStore", "2048").toInt

    // local database and zookeeper are used to support transaction, which is critical for exactly once auditing
    val requireExactlyOnce = chaperoneProps.getProperty("requireExactlyOnce", "true").toBoolean
    val jdbcDriver = chaperoneProps.getProperty("jdbcDriver", "org.hsqldb.jdbc.JDBCDriver")
    val dbUrl = chaperoneProps.getProperty("dbUrl", "jdbc:hsqldb:file:/tmp/chaperone/auditmsg-store/" + hostMeta.asIdentity)
    val dbUser = chaperoneProps.getProperty("dbUser", "cs")
    val dbPass = chaperoneProps.getProperty("dbPass", "cs")
    val dbServerPort = chaperoneProps.getProperty("dbServerPort", "17011").toInt
    val tableName = chaperoneProps.getProperty("tableName", "INTERVAL_REPORT")
    val auditMsgRetentionMs = TimeUnit.HOURS.toMillis(chaperoneProps.getProperty("auditMsgRetentionHr", "1").toLong)
    val resendLocalBatchSize = chaperoneProps.getProperty("resendLocalBatchSize", "64").toInt
    val resendLocalIntervalInSec = chaperoneProps.getProperty("resendLocalIntervalInSec", "10").toInt
    val resendLocalDelayInSec = chaperoneProps.getProperty("resendLocalDelayInSec", "10").toInt
    val resendZkIntervalInMs = chaperoneProps.getProperty("resendZkIntervalInMs", "1000").toInt
    val resendZkDelayInMs = chaperoneProps.getProperty("resendZkDelayInMs", "10000").toInt
    val zkServers = chaperoneProps.getProperty("zkServers", "localhost:2181")
    val zkConnectionTimeoutMs = consumerConfig.zkConnectionTimeoutMs
    val zkSessionTimeoutMs = consumerConfig.zkSessionTimeoutMs
    val zkGroupId = consumerConfig.groupId

    // initialize Chaperone
    val auditMsgReporter = new AuditMsgReporter(hostMeta, auditTopicName, producer.producer)

    val currentEpochInMs = System.currentTimeMillis()
    zkOffsetStore = new ZkOffsetStore(currentEpochInMs, zkServers, zkConnectionTimeoutMs, zkSessionTimeoutMs, zkGroupId, hostMeta)
    val auditMsgStore = new AuditMsgStore(currentEpochInMs, jdbcDriver, dbUrl, dbUser, dbPass, dbServerPort, tableName)
    verificationStore = new VerificationStore(zkOffsetStore, auditMsgStore, auditMsgReporter, auditMsgRetentionMs,
      resendLocalBatchSize, resendLocalDelayInSec, resendLocalIntervalInSec,
      resendZkDelayInMs, resendZkIntervalInMs, queueSizeForVerificationStore, hostMeta, requireExactlyOnce)
    verificationStore.open()

    auditors = new Array[MessageAuditor](auditorCount)
    for (idx <- 0 to auditorCount - 1) {
      auditors(idx) = new MessageAuditor(idx, JsonTopTsExtractor, queueSizeForMessageAuditor, initialMapSize, timeBucketIntervalInSec,
        reportFreqBucketCount, reportFreqIntervalInMs, verificationStore)
      auditors(idx).start()
    }
  }
  ...
  def isShuttingDown(): Boolean = isShuttingdown.get()
  ...
  def cleanShutdown() {
    if (isShuttingdown.compareAndSet(false, true)) {
      ...
      info("Shutting down messageAuditors")
      for (idx <- 0 to auditorCount - 1) {
        auditors(idx).shutdown()
      }

      for (auditor <- auditors) {
        auditor.shutdown()
      }

      info("Closing VerificationStore")
  ...
  class MirrorMakerThread(connector: KafkaConnector,
    ...
    // msgs is dispatched by its topicName to be processed in parallel
    def auditMessage(msg: MessageAndMetadata[Array[Byte], Array[Byte]]): Boolean = {
      val idx = Math.abs(msg.topic.hashCode + msg.partition) % auditorCount
      debug("Dispatch msg from topic=%s to auditor[%d]".format(msg.topic, idx))
      auditors(idx).submitMessage(msg)
      true
    }

    def maybeFlushAndCommitOffsets() {
      if (!doMsgAudit && System.currentTimeMillis() - lastOffsetCommitMs > offsetCommitIntervalMs) {
        producer.flush()
        commitOffsets(connector)
